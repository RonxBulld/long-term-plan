...
export interface RepairResult {
  newText: string;
  applied: {
    addFormatHeader: boolean;
    addMissingIds: number;
  };
}
...
function addFormatHeader(lines: string[]): boolean {
...
function addMissingIds(lines: string[]): number {
  let added = 0;
  let inFence = false;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!line.match(TASK_LINE_MISSING_ID_RE)) continue;
    if (line.match(TASK_ID_TRAILER_RE)) continue;
